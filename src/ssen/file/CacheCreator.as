...
public class CacheCreator implements IDeconstructable {
	...
	********************************************************************* */
	 ...
	 */
	public function create(callback:Function, bytesStart:Number = -1,
						   divide:Number = 0):void {
		if (cache) {
			cache.deleteFile();
		}
	...
	private function read(event:ProgressEvent):void {
		var length:Number = (_reader.bytesAvailable >= _read) ? _read :
			_reader.bytesAvailable;
